...
本文提取出OAuth2.0规范RFC6749的主要内容，部分内容从文档复制出来，讲讲第三方授权。
...
![](/assets/images/2016-06-19-oauth2.0-2-1.png)
图2


因为我已经登录了，所以直接点上面的QQ头像直接授权。
...
1. 首先，用户点击QQ登录时，知乎服务器会帮我们重定向到QQ的第三方授权服务器上页面上(图3)请求授权，同时，跟随一个重定向URL指向知乎服务器的某个地址，我们点击授权后(图3)，授权成功，重定向到知乎指定的URL并返回授权码(可以在URL看到)
...
- 若包括了客户端身份验证，验证客户端身份，确保授权码颁发给了通过身份验证的机密客户端，或者如果客户端是公开的，确保代码颁发给了请求
中的“client_id”，
- 验证授权码是有效的，并确保给出了“redirect_uri”参数，若“redirect_uri”参数如 4.1.1 所述包含在初始授权请求中，且若包含，确保它们的值是相同的。
...
![](/assets/images/2016-06-19-oauth2.0-6.png)
